...
if (isset($_POST['interface'])) {
    ...
    if ( $int == "mobiledata") {
        ...
        if(!isset($_POST['opts']) ) {
            ...
            $newmac=trim(strtolower($_POST["int-new-mac-".$dev]));
            ...
            // change MAC if addresses differ
            if (strcmp($mac, $newmac) !== 0) {
                exec('sudo ip link set ' .$dev. ' down');
                sleep(1);
                exec('sudo ip link set ' .$dev. ' address '.$newmac);
                exec('sudo ip link set ' .$dev. ' up');
                $mac = $newmac;
                $macAddrChange = true;
            }
            ...
            if ($macAddrChange) {
                $jsonData = ['return'=>0,'output'=>['Address changed for device '.$dev. '<br>New MAC address: '.$newmac ] ];
            } else {
