...
function DisplayWireGuardConfig()
{
    ...
    if (!RASPI_MONITOR_ENABLED) {
        ...
        if (isset($_POST['savewgsettings']) && $optConf == 'manual' && $optSrvEnable == 1 ) {
            ...
            SaveWireGuardUpload($status, $_FILES['wgFile'], $optRules, $optKSwitch);
 ...
 */
function SaveWireGuardUpload($status, $file, $optRules, $optKSwitch)
{
    define('KB', 1024);
    $tmp_destdir = '/tmp/';
    $auth_flag = 0;
    ...
    try {
        ...
        if ((isset($optRules) || isset($optKSwitch)) && preg_match('/PostUp|PostDown|PreDown/m',$tmp_contents)) {
            ...
            if (isset($optKSwitch)) {
                ...
                $jsonData = json_decode(getNetConfig($_SESSION['ap_interface']), true);
                $ip_addr = ($jsonData['StaticIP'] == '') ? getDefaultNetValue('dhcp','wlan0','static ip_address') : $jsonData['StaticIP'];
                $mask = ($jsonData['SubnetMask'] == '') ? getDefaultNetValue('dhcp','wlan0','subnetmask') : $jsonData['SubnetMask'];
                $rules[] = 'PostUp = '.getDefaultNetValue('wireguard','server','PostUpEx');
                $rules[] = 'PreDown = '.getDefaultNetValue('wireguard','server','PreDown');
                $rules = preg_replace('/%s/m', $ip_addr.'/'.mask2cidr($mask), $rules);
            }
            if (count($rules) > 0) {
                $rules[] = '';
